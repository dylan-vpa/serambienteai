...
JSON:`;
    ...
    extractOITDataHeuristic(documentText) {
        ...
        let status = 'PENDING';
        if (lower.includes('completado') || lower.includes('terminado'))
            status = 'COMPLETED';
        else if (lower.includes('en curso') || lower.includes('progreso'))
            status = 'IN_PROGRESS';
        const valid = !!oitNumber;
        const errors = valid ? [] : ['No se encontró número de OIT'];
        const warnings = [];
        if (!description || description.length < 10)
            warnings.push('Descripción incompleta');
        return {
            valid,
     ...
     */
    processOITInBackground(oitId, oitFilePath, quotationFilePath, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            console.log(`🚀 [AI SERVICE] Iniciando procesamiento OIT ${oitId}`);
            try {
                const extractText = (filePath) => __awaiter(this, void 0, void 0, function* () {
                    const pdfService = (yield Promise.resolve().then(() => __importStar(require('../services/pdf.service')))).pdfService;
                    return yield pdfService.extractText(filePath);
                });
                ...
                // Create notification
                if ((_a = aiDataContent.data) === null || _a === void 0 ? void 0 : _a.proposedDate) {
